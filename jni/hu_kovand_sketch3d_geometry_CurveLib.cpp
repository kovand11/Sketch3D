...
#include "hu_kovand_sketch3d_geometry_CurveLib.h"
...
{
  std::pair<std::vector<double>::const_iterator,std::vector<double>::const_iterator> range = std::equal_range(knots.begin(), knots.end(), u);
...
JNIEXPORT jdoubleArray JNICALL Java_hu_kovand_sketch3d_geometry_CurveLib_approximate(JNIEnv *env, jclass cl, jdoubleArray px, jdoubleArray py, jdoubleArray pz, jint p, jint n)
{
	jsize size = env->GetArrayLength(px);

	std::vector<double> px_vec(size);
	std::vector<double> py_vec(size);
	std::vector<double> pz_vec(size);

	env->GetDoubleArrayRegion(px,0,size,&px_vec[0]);
	env->GetDoubleArrayRegion(py,0,size,&py_vec[0]);
	env->GetDoubleArrayRegion(pz,0,size,&pz_vec[0]);

	PointVector points;
	for (jsize i=0;i<size;i++)
	{
		points.push_back(Point(px_vec[i],py_vec[i],pz_vec[i]));
	}

	BSplineCurve curve = BSplineCurve::approximate(points,p,n);

	std::vector<jdouble> results;
		//               //knots///   //cp////////
	results.reserve( n + p + 2 + 2 *( n + 1 ));
	for (int i=0;i<curve.knots.size();i++)
	{
		results.push_back(curve.knots[i]);
	}
	for (int i=0;i<curve.cp.size();i++)
	{
		results.push_back(curve.cp[i].x);
		results.push_back(curve.cp[i].y);
		results.push_back(curve.cp[i].z);
	}
...
JNIEXPORT jdoubleArray JNICALL Java_hu_kovand_sketch3d_geometry_CurveLib_evaluateN(JNIEnv *env, jclass cl, jdoubleArray knots, jdoubleArray cpx, jdoubleArray cpy, jdoubleArray cpz, jint points)
{
	...
	//points must be >2
	std::vector<jdouble> results;
	results.reserve(3*points);
	for (int i=0;i<points;i++)
	{
		Point p = curve.evaluate(i/(points-1.0));
		results.push_back(p.x);
		results.push_back(p.y);
		results.push_back(p.z);
	}
...
JNIEXPORT jdoubleArray JNICALL Java_hu_kovand_sketch3d_geometry_CurveLib_evaluate(JNIEnv *env, jclass cl, jdoubleArray knots, jdoubleArray cpx, jdoubleArray cpy, jdoubleArray cpz, jfloat t)
{
	jsize knots_size = env->GetArrayLength(knots);
	jsize cp_size = env->GetArrayLength(cpx);
